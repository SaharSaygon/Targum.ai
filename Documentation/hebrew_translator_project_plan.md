...
- **Tools** (built — **8**): `list_folder`, `read_file`, `translate_text_pdf`, `translate_image_pdf`, `save_to_vault`, `update_mapping`, `fetch_signal_detail` (retrieves offloaded verbose detector signals on demand), and `skip_file` (records a deliberate agent skip as `skipped_permanent` with `source_md5` — **added 2026-06-06 with the pre-pass; tool count 7 → 8**; see HISTORY). ~~`ask_user`~~ — **REMOVED 2026-06-02 (full autonomy, see HISTORY "Day 2 Part 4")**: no approval/interaction gate; routing + naming uncertainty is handled by the agent itself, with genuine cannot-determine cases skipped + logged. (`flag_for_approval`, briefly considered, was also removed.) Tool design = real architecture work; the system prompt = the policy spec. Text and image translation are separate tools — mode dispatch IS the tool call (see "Mode Dispatch — Agent-Driven via Signals" below).
- **Deterministic pre-pass (`prepass.py`, built 2026-06-06)**: a pure, `anthropic`-free pre-step that recursively walks the Drive tree via `list_folder_children` (md5 metadata only, no downloads), diffs each file against the manifest via `dedup.py`, and emits a **worklist of only new/changed files** — unchanged files (incl. already-marked skips) are absent. The kickoff hands the loop the worklist instead of a root folder ID (`while True` → `while worklist`); an empty worklist ends the run with no spend. The agent loop and its tools are unchanged. See HISTORY "Deterministic routing pre-pass built + skip-rule narrowed (2026-06-06)".
- **Drive structure (typical, not enforced)**: Course folder contains Lectures/Tutorials as flat files at top level. Exams and Homework live in container folders, often split into `נקי` (clean → translate) and `פתור` (solved → skip *only* when under an עבוד-homework folder — see the SKIP reversal in Rules; elsewhere `פתור`-named files now translate).
- **Semantic role classification (in system prompt)**:
  ...
  - ~~SKIP: פתור, פתרון, תשובות, מענה~~ — **REVERSED 2026-06-06 (see HISTORY "Deterministic routing pre-pass built + skip-rule narrowed")**: the broad keyword-skip (those tokens matching ANY path segment) is GONE. The ONLY skip-by-pattern case is an unreadable handwritten **homework solution**: a solution segment (stem פתר, e.g. פתור/פתרון) nested **under** a homework folder (stem עבוד, e.g. עבודות/עבודות בית) — BOTH stems required. The agent records it via `skip_file` (→ `skipped_permanent`, with `source_md5`); the pre-pass drops it on later runs. Original intent was only "skip my unreadable handwritten homework solutions"; the broad rule over-reached. Solution-like names OUTSIDE the עבוד-homework context now translate normally. No skip logic in the pre-pass.
  - CLEAN: נקי, ריק, ללא פתרון, שאלות (inherits type from parent container)
- **Rules**:
  - ~~Skip wins — any segment of the path matching SKIP → don't translate~~ — **REVERSED 2026-06-06 (see HISTORY "Deterministic routing pre-pass built + skip-rule narrowed")**: no broad segment match, and no skip rule in the pre-pass. Skip is the agent's decision on NEW files only — a פתר-solution segment under an עבוד-homework folder → record via `skip_file` (`skipped_permanent`), so the pre-pass drops it for free next run. Distinct from the run-log-only skip-floor (genuine cannot-determine, no manifest entry).
